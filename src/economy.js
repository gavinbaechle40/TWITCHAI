...
const GAMBLE_COOLDOWN_MS = Number(process.env.KOEN_GAMBLE_COOLDOWN_MS || 60000);
const BLACKJACK_SESSION_MS = Number(process.env.KOEN_BLACKJACK_SESSION_MS || 5 * 60 * 1000);
const HAT_COLOR_COST = Number(process.env.KOEN_HAT_COLOR_COST || 10000);
...
function migrateDB(db) {
  ...
  db.settings.gamblingPaused = Boolean(db.settings.gamblingPaused);
...
function gamblingPausedText(db) {
  return db.settings?.gamblingPaused ? "Koen gambling is paused right now. Mods can use !abi pause off to resume." : null;
}
...
function isKoenCommand(command) {
  return [
    ...
    "roulette", "roul", "rob", "give", "level", "perks", "colors", "color",
    "title", "titles", "pause", "commands", "help"
  ].includes(command);
}
...
export async function handleKoenCommand({ client, channel, tags, message, say, userState }) {
  ...
  if (command === "perks") {
    response = `Koen perks: L5 better rob odds, L15 slightly better bets, L20 VIP flex. You are L${viewer.level}: ${getLevelPerks(viewer.level)}.`;
  }

  if (command === "commands" || (command === "help" && String(args[0] || "").toLowerCase() === "koen")) {
    response = `Koen commands: !abi balance, give @user <amount|all|50%>, bet <amount>, spin <amount>, roulette red <amount|all|50%>, roulette number 17 <amount|all|50%>, blackjack <amount|all|50%>, hit, stand, double, split, duck <amount|all|50%>, duck step, duck cashout, rob @user, koen leaderboard, level, perks, colors, color <color>, title list, title buy <name>, title custom <your title>, title clear. Roulette supports red/black/green, even/odd, low/high, dozen1-3, column1-3, and number 0-36. All Koen gambling/robbing cooldowns are ${Math.ceil(GAMBLE_COOLDOWN_MS / 1000)}s. Hat colors cost ${format(HAT_COLOR_COST)} Koen and only work with !abi color <color>. Preset titles cost ${format(TITLE_COST)}, custom titles cost ${format(CUSTOM_TITLE_COST)}. Mods can use !abi pause to pause/resume gambling. Use !abi koen leaderboard for the leaderboard. Type !join when jackpot opens.`;
  }

  if (command === "pause") {
    const action = String(args[0] || "").toLowerCase();
    if (!isModOrBroadcaster(tags)) {
      response = "only mods/broadcaster can pause or resume Koen gambling.";
    } else {
      if (["on", "pause", "paused"].includes(action)) db.settings.gamblingPaused = true;
      else if (["off", "resume", "unpause", "unpaused"].includes(action)) db.settings.gamblingPaused = false;
      else db.settings.gamblingPaused = !db.settings.gamblingPaused;

      response = db.settings.gamblingPaused
        ? "Koen gambling is now PAUSED. Balance, give, titles, and colors still work."
        : "Koen gambling is now RESUMED.";
    }
  }

  if (command === "give") {
    const targetName = normalizeName(args[0]);
    const amount = resolveWager(viewer, args[1], 0);

    if (!targetName) response = "usage: !abi give @viewer 5000";
    else if (targetName === username) response = "you cannot give Koen to yourself.";
    else if ((config.botAliases || []).includes(targetName)) response = "you cannot give Koen to the bot bank.";
    else if (amount <= 0) response = "usage: !abi give @viewer <amount|all|50%|half>";
    else if (viewer.koen < amount) response = `you only have ${format(viewer.koen)} Koen.`;
    else {
      const target = getUser(db, targetName);
      viewer.koen -= amount;
      target.koen += amount;
      viewer.koenGiven = Number(viewer.koenGiven || 0) + amount;
      target.koenReceived = Number(target.koenReceived || 0) + amount;
      awardXp(viewer, Math.floor(amount / 100));
      response = `@${username} gave ${format(amount)} Koen to @${targetName}. Your balance: ${format(viewer.koen)}.`;
    }
  }
  ...
  if (command === "rob") {
    ...
    if (paused) response = paused;
    else if (!targetName) response = "usage: !abi rob @viewer";
